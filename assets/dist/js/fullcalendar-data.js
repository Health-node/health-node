...
$(document).ready(function () {
    ...
    var patient_data = [];
    $.ajax({
        url: 'get_calender_appointment_list',
        success: function (data) {
            let data1 = JSON.parse(data);
            data1.forEach(function (item) {
                var start = new Date(item.AppointmentTime);
                let hours = start.getHours();
                let minutes = start.getMinutes();
                let appointmenttime = hours + ':' + minutes;
                let appointmentdate = moment(start).format('YYYY-MM-DD');
                let appointment = {
                    title: "Appointment",
                    start: appointmentdate,
                    time: appointmenttime
                };
                patient_data.push(appointment);
            });
            $('#patient_calender').fullCalendar({
                header: {
                ...
                eventMouseover: function (data, event, view) {
                    var tooltip = '<div class="tooltiptopicevent tooltip tooltip-inner" style="width:auto;height:auto;position:absolute;z-index:10001;"> '+ 'Appointment time: ' + data.time + '</div>';
                    $("body").append(tooltip);
